...
var cdCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// derivation), with a legacy gh fallback for URL-shaped inputs that
		// have no state entry. The state-recorded workdir wins because, for
		// workflow-setup sessions, it is declared by the setup script and
		// not derivable from the path convention.
