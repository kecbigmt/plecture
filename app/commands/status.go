...
provider-specific interpretation:
  ...
  work      each task instance's outputs (dynamic and mutable alike),
            ...
            facts "tws tick" acts on and "tws check" used to report
...
output (most github-provider workflows do, for done_when). "tws ls" still
carries the legacy GitHub summary line for sessions predating that.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if statusJSON {
			...
			// this command's output with `2>&1` (e.g. config/tws/providers/
