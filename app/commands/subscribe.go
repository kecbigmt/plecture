...
state changes — readable with 'tws event list').
...
($TWS_SESSION_NAME, exported into the agent's shell), so a running agent can
simply 'tws subscribe <url>'. --session overrides it.`,
