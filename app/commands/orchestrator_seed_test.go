...
// outside the tws Nix package's fileset, so resolve from the repo root and
...
func TestOrchestratorTickTemplate_IsMinimal(t *testing.T) {
	...
	tickPath := filepath.Join(repoRoot, "config", "tws", "templates", "orchestrator_tick.md")
	tick := readOrSkip(t, tickPath)
	fullPath := filepath.Join(repoRoot, "config", "tws", "templates", "orchestrator.md")
