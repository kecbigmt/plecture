...
func writeCatalogList(out io.Writer, entries []service.CatalogListEntry) error {
	...
	fmt.Fprintln(w, "ALIAS\tSOURCE\tREVISION\tSTATUS\tPLUGINS")
	for _, e := range entries {
		fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%v\n", e.Alias, e.Source, orNone(e.ResolvedRevision), e.Status, e.EnabledPlugins)
	}
	return w.Flush()
}
...
func runCatalogAdd(cmd *cobra.Command, alias, source string) error {
	...
	params := service.CatalogAddParams{Alias: alias, Source: source, Revision: catalogAddRevision}
