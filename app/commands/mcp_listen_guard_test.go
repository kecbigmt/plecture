...
// `self` executable mcpserver.Listen re-execs as "plecture mcp serve" — go test
// has no separate plecture binary to point mcpListenCmd's os.Executable() lookup
// at, so the test binary re-execs itself (the same helper-process pattern
// os/exec_test.go uses).
const helperProcessEnv = "PLECTURE_MCP_LISTEN_TEST_HELPER_PROCESS"
...
// PLECTURE_SESSION_GUARD of its own, so a rejection can only come from the
...
func TestMCPListen_ScopesSessionGuardToOwnSession(t *testing.T) {
	...
	t.Run("in scope: own session", func(t *testing.T) {
		body := connect().callTool(t, "plecture_event_publish", map[string]any{
