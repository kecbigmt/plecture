...
func init() {
	...
	for _, cmd := range []*cobra.Command{approveCmd, requestChangesCmd} {
		...
		cmd.Flags().StringVar(&judgeReviewerSession, "reviewer-session", "", "Reviewer session name (defaults to $TWS_SESSION_NAME; provenance-constrained judges require it to match the ambient reviewer pane)")
