...
var mcpListenCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		// --session injects TWS_SESSION_GUARD into every spawned connection
...
// contains "/" (e.g. "owner/repo-123"); filepath.Join turns that into nested
...
func defaultSessionMcpListenSocket(sessionName string) string {
	...
	return filepath.Join(rt, "tws-mcp", sessionName+".sock")
}

func init() {
	mcpListenCmd.Flags().StringVar(&mcpListenSocket, "socket", "", "Unix socket path to listen on (default: the --session convention path)")
	mcpListenCmd.Flags().StringVar(&mcpListenSession, "session", "", "Scope this socket to a session: injects TWS_SESSION_GUARD for it and its own name-space into every spawned connection, and derives --socket's default")
