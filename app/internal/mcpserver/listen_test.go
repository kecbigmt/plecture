...
func TestListenSetsSocketPermissions(t *testing.T) {
	...
	socket := filepath.Join(t.TempDir(), "tws.sock")
...
func TestMCPListenHelperProcess(t *testing.T) {
	if os.Getenv("TWS_MCP_LISTEN_HELPER") != "1" {
		return
	}
	serveHelperMCP()
	os.Exit(0)
...
func writeHelperExecutable(t *testing.T) string {
	t.Helper()
	path := filepath.Join(t.TempDir(), "tws-helper")
	script := fmt.Sprintf("#!/bin/sh\nTWS_MCP_LISTEN_HELPER=1 exec %q -test.run=TestMCPListenHelperProcess -- \"$@\"\n", os.Args[0])
