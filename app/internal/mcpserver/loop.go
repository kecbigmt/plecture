...
var taskFinalizeTool = mcp.NewTool("sennit_task_finalize",
	mcp.WithDescription("Finalize a task instance (ADR \"goal-as-task\" D4): reconfirm its done_when is satisfied at the current revision (refusing, with no record made, if it is not), then let the bound --resource's definition record completion via its finalize script if it declares one. Gate + record only — the instance is left in place; run sennit_task_cleanup separately afterward to reclaim it. A resource definition without a finalize script is a no-op step, not an error."),
	mcp.WithString("instance",
		mcp.Required(),
...
var tickTool = mcp.NewTool("sennit_tick",
...
var watchdogCheckTool = mcp.NewTool("sennit_watchdog_check",
...
var judgeApproveTool = mcp.NewTool("sennit_judge_approve",
	...
	mcp.WithString("reviewer_session",
		mcp.Description("Reviewer session name (defaults to SENNIT_SESSION_NAME; provenance-constrained judges require it to match the ambient reviewer pane)"),
	),
)

var judgeRequestChangesTool = mcp.NewTool("sennit_judge_request_changes",
...
var subscribeTool = mcp.NewTool("sennit_subscribe",
	mcp.WithDescription("Subscribe a live session to an opaque resource so its events (for GitHub PRs/issues: CI status, review decisions, state changes) arrive in this session's event stream, readable with sennit_event_list. Additive — never replaces existing subscriptions, and subscribing a resource another session already watches does not take it over."),
	mcp.WithString("resource",
		mcp.Required(),
		mcp.Description("Resource id to subscribe to (e.g. a GitHub Issue or PR URL)"),
	),
	mcp.WithString("session",
		mcp.Description("Subscriber session name (defaults to SENNIT_SESSION_NAME)"),
	),
)
...
func handleTaskFinalize(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	return jsonResult(map[string]any{
		...
		"next_step":           fmt.Sprintf("run sennit_task_cleanup on %q to reclaim it", result.Instance),
	})
}
