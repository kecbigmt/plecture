...
// NewServer creates a new MCP server with tws tools registered.
func NewServer() *server.MCPServer {
	...
	s.AddTools(
		server.ServerTool{Tool: createTool, Handler: wrap("tws_create", handleCreate)},
		server.ServerTool{Tool: upTool, Handler: wrap("tws_up", handleUp)},
		server.ServerTool{Tool: downTool, Handler: wrap("tws_down", handleDown)},
		server.ServerTool{Tool: destroyTool, Handler: wrap("tws_destroy", handleDestroy)},
		server.ServerTool{Tool: statusTool, Handler: wrap("tws_status", handleStatus)},
		server.ServerTool{Tool: captureTool, Handler: wrap("tws_capture", handleCapture)},
		server.ServerTool{Tool: listTool, Handler: wrap("tws_list", handleList)},
		server.ServerTool{Tool: templateListTool, Handler: wrap("tws_template_list", handleTemplateList)},
		server.ServerTool{Tool: workflowListTool, Handler: wrap("tws_workflow_list", handleWorkflowList)},
		server.ServerTool{Tool: workflowShowTool, Handler: wrap("tws_workflow_show", handleWorkflowShow)},
		server.ServerTool{Tool: gcTool, Handler: wrap("tws_gc", handleGC)},
		server.ServerTool{Tool: eventListTool, Handler: wrap("tws_event_list", handleEventList)},
		server.ServerTool{Tool: eventShowTool, Handler: wrap("tws_event_show", handleEventShow)},
		server.ServerTool{Tool: eventPublishTool, Handler: wrap("tws_event_publish", handleEventPublish)},
		server.ServerTool{Tool: taskSetupTool, Handler: wrap("tws_task_setup", handleTaskSetup)},
		server.ServerTool{Tool: taskCleanupTool, Handler: wrap("tws_task_cleanup", handleTaskCleanup)},
		server.ServerTool{Tool: taskFinalizeTool, Handler: wrap("tws_task_finalize", handleTaskFinalize)},
		server.ServerTool{Tool: resourceStatusTool, Handler: wrap("tws_resource_status", handleResourceStatus)},
		server.ServerTool{Tool: checkTool, Handler: wrap("tws_check", handleCheck)},
		server.ServerTool{Tool: tickTool, Handler: wrap("tws_tick", handleTick)},
		server.ServerTool{Tool: watchdogCheckTool, Handler: wrap("tws_watchdog_check", handleWatchdogCheck)},
		server.ServerTool{Tool: judgeApproveTool, Handler: wrap("tws_judge_approve", handleJudgeApprove)},
		server.ServerTool{Tool: judgeRequestChangesTool, Handler: wrap("tws_judge_request_changes", handleJudgeRequestChanges)},
		server.ServerTool{Tool: subscribeTool, Handler: wrap("tws_subscribe", handleSubscribe)},
	)

	return s
}

var createTool = mcp.NewTool("tws_create",
	mcp.WithDescription("Create a session for a resource identifier and run session-scoped tasks. A workflow whose [resolver] matches the identifier is auto-selected (GitHub issue/PR URLs and PVTI_xxx project items today); other identifiers need an explicit workflow. Does not start the runtime — call tws_up to launch run-scoped tasks."),
	mcp.WithString("url",
		mcp.Required(),
	...
	mcp.WithString("parent",
		mcp.Description("Parent session name for the session tree. Defaults to TWS_SESSION_NAME when it names an existing session."),
	),
...
var upTool = mcp.NewTool("tws_up",
	mcp.WithDescription("Run run-scoped tasks for a session, in dependency-respecting order. When the identifier is a resource identifier and no state entry exists (or a session-scoped task has not yet reached \"produced\"), tws_create is invoked first (docker compose up-style auto-create)."),
	mcp.WithString("session",
		mcp.Required(),
		mcp.Description("Resource identifier (e.g. a GitHub Issue/PR URL) or session name (e.g. owner/repo-123)"),
	),
	mcp.WithString("tag",
		mcp.Description("Workspace-identity label of the session to resolve/auto-create (resource-identifier only, not a bare session name). Defaults to the workflow id, matching tws_create."),
	),
	mcp.WithString("parent",
		mcp.Description("Parent session name for auto-created sessions. Defaults to TWS_SESSION_NAME when it names an existing session."),
	),
	mcp.WithObject("inputs",
...
var downTool = mcp.NewTool("tws_down",
...
var destroyTool = mcp.NewTool("tws_destroy",
...
var statusTool = mcp.NewTool("tws_status",
...
var captureTool = mcp.NewTool("tws_capture",
...
var listTool = mcp.NewTool("tws_list",
...
var templateListTool = mcp.NewTool("tws_template_list",
...
var workflowListTool = mcp.NewTool("tws_workflow_list",
	mcp.WithDescription("List workflows discoverable via the .tws/workflows cascade. Each entry surfaces id/name/description so an agent can pick the right workflow before calling tws_workflow_show or tws_create."),
	mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: boolPtr(true)}),
)

var workflowShowTool = mcp.NewTool("tws_workflow_show",
	mcp.WithDescription("Return full details for a single workflow: id, name, description, the (merged) inputs_schema, and the compiled DAG of nodes. Use inputs_schema to build the --inputs payload for tws_create / tws_up."),
	mcp.WithToolAnnotation(mcp.ToolAnnotation{ReadOnlyHint: boolPtr(true)}),
	mcp.WithString("id",
		mcp.Required(),
