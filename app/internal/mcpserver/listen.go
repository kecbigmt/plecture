...
// stdio `plect mcp serve` child process. A non-empty guard makes this socket's
// scope independent of whatever PLECT_SESSION_GUARD the listening process
// itself inherited.
...
func handleConnection(ctx context.Context, self string, conn net.Conn, guard string) {
	...
	if guard != "" {
		cmd.Env = envWithOverride(os.Environ(), "PLECT_SESSION_GUARD", guard)
	}
