...
func TestInstrumentHandler_Success(t *testing.T) {
	...
	handler := instrumentHandler("plect_list", inner)
	...
	assertField(t, rec, "component", "plect-mcp")
	assertField(t, rec, "tool", "plect_list")
...
func TestInstrumentHandler_ToolError(t *testing.T) {
	...
	handler := instrumentHandler("plect_up", inner)
	...
	assertField(t, rec, "tool", "plect_up")
	...
	// from it is the workspace provider resolver's job, not the log
	// wrapper's.
...
func TestInstrumentHandler_TraceIDContinuity(t *testing.T) {
	...
	handler := instrumentHandler("plect_destroy", inner)
	req := mcp.CallToolRequest{}
	req.Params.Arguments = map[string]any{
