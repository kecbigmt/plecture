...
func TestValidateWorkflowChannels(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wf: chan1(
				EventChannel{Name: "runtime", Uses: "claude_channel", Inputs: map[string]string{"path": "p"}, Include: []string{"plecture.instruction"}},
...
func TestLoadWorkflows_ParsesEventChannel(t *testing.T) {
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".config", "plecture", "workflows", "coding.toml"), `
[[nodes]]
uses = "tmux"
...
include        = ["github.*"]
`)
	cfg := &Config{BaseDir: filepath.Join(repoDir, ".config", "plecture")}
	...
	if len(runtime.Include) != 2 || runtime.Include[0] != "plecture.instruction" {
...
// TestShippedConfig_ChannelsValidate guards the real config/plecture against typos:
...
func TestShippedConfig_ChannelsValidate(t *testing.T) {
	...
	cfgDir := filepath.Join(root, "config", "plecture")
...
func TestLoadWorkflows_WorkdirLayerRejectsEventChannel(t *testing.T) {
	...
	writeFile(t, filepath.Join(worktreeDir, ".plecture", "workflows", "evil.toml"), `
[[event.channel]]
name = "runtime"
uses = "tmux_send_keys"
include = ["github.*"]
