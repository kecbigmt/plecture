...
func TestLoadWorkflows_RepoDir(t *testing.T) {
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".tws", "workflows", "coding-claude.toml"), `
[[nodes]]
id = "tmux"
uses = "tmux"
...
`)
	...
	writeFile(t, filepath.Join(worktreeDir, ".tws", "workflows", "shared.toml"), `
[[nodes]]
id = "session_extra"
`)
	cfg := Load()
...
id = "o"
`)
	writeFile(t, filepath.Join(repoDir, ".tws", "workflows", "shared.toml"), `
[[nodes]]
id = "r"
`)
	writeFile(t, filepath.Join(sessionDir, ".tws", "workflows", "shared.toml"), `
[[nodes]]
id = "s"
`)
	cfg := Load()
...
func TestLoadWorkflows_CascadeStopsAtHome(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	writeFile(t, filepath.Join(tmpHome, ".tws", "workflows", "leak.toml"), `
[[nodes]]
id = "should_not_load"
`)
	worktreeDir := filepath.Join(tmpHome, "worktrees", "session")
	...
	if _, ok := got["leak"]; ok {
		t.Fatalf("workflow under $HOME/.tws was loaded; cascade should stop before $HOME")
	}
}
...
func TestLoadWorkflows_DifferentFilenamesStayIndependent(t *testing.T) {
	...
	t.Setenv("HOME", tmpHome)
	worktreeDir := filepath.Join(tmpHome, "worktrees", "session")
	writeFile(t, filepath.Join(worktreeDir, ".tws", "workflows", "a.toml"), `
[[nodes]]
id = "a_node"
`)
	writeFile(t, filepath.Join(worktreeDir, ".tws", "workflows", "b.toml"), `
[[nodes]]
id = "b_node"
`)
	cfg := &Config{}
...
func TestLoadWorkflows_InvalidStemRejected(t *testing.T) {
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".tws", "workflows", "-bad.toml"), `
[[nodes]]
id = "tmux"
`)
	cfg := &Config{}
...
func TestLoadTaskDefinitions(t *testing.T) {
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".tws", "tasks", "tmux.toml"), `
scope = "run"
setup = "echo '{}'"
`)
...
func TestLoadTaskDefinitions_InjectsBuiltinWorktreeDirty(t *testing.T) {
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".tws", "tasks", "work.toml"), `
scope = "run"
setup = "echo '{}'"
[[done_when.all]]
...
`)
	...
	writeFile(t, filepath.Join(repoDir, ".tws", "tasks", "a.toml"), `
scope = "session"
...
func TestLoadTaskDefinitions_HyphenStemRejected(t *testing.T) {
	...
	repoDir := t.TempDir()
	writeFile(t, filepath.Join(repoDir, ".tws", "tasks", "slack-thread.toml"), `
scope = "session"
setup = "true"
`)
	worktreeDir := filepath.Join(repoDir, "session")
...
func TestLoadTaskDefinitions_WorkdirLayerRejected(t *testing.T) {
	...
	worktreeDir := filepath.Join(tmpHome, "worktrees", "github.com", "org", "repo", "session")
	writeFile(t, filepath.Join(worktreeDir, ".tws", "tasks", "evil.toml"), `
...
func TestLoadTaskDefinitions_AncestorLayerStillTrusted(t *testing.T) {
	...
	repoDir := filepath.Join(tmpHome, "worktrees", "github.com", "org", "repo")
	writeFile(t, filepath.Join(repoDir, ".tws", "tasks", "teardown.toml"), `
setup = "echo '{}'"
`)
	worktreeDir := filepath.Join(repoDir, "session")
	if err := os.MkdirAll(worktreeDir, 0o755); err != nil {
