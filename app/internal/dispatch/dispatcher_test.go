...
func runTestDispatcher(t *testing.T, log *eventlog.Store, sock string) (*sessionDispatcher, *state.Store, *sessionhub.Registry) {
	...
	d := &sessionDispatcher{
		session:  "o/r-1",
		channels: []config.EventChannel{{Name: "runtime", Uses: "claude_channel", Inputs: map[string]string{"path": "{{.Nodes.claude.outputs.socket_path}}"}, Include: []string{"tws.instruction"}}},
...
func TestDispatcher_DeliversIncludedEvents(t *testing.T) {
	log := eventlog.NewStore(t.TempDir())
	sock, recv := startFakeSocket(t)
	d, s := runtimeDispatcher("o/r-1", log, sock, "tws.instruction", "github.*")
...
func TestDispatcher_IncludeFilters(t *testing.T) {
	log := eventlog.NewStore(t.TempDir())
	sock, recv := startFakeSocket(t)
	d, s := runtimeDispatcher("o/r-1", log, sock, "tws.instruction")
...
func TestDispatcher_FinalFailureAppendsChannelError(t *testing.T) {
	...
	d, s := runtimeDispatcher("o/r-1", log, dead, "tws.instruction")
	...
	if len(errs) != 1 {
		t.Fatalf("want exactly one tws.channel.error, got %d", len(errs))
	}
	ce := errs[0]
...
func TestDispatcher_ChannelErrorNotRedelivered(t *testing.T) {
	...
	d, s := runtimeDispatcher("o/r-1", log, dead, "tws.instruction", "github.*")
...
func TestDispatcher_MultiChannelFanOut(t *testing.T) {
	...
	d := &sessionDispatcher{
		session: "o/r-1",
		channels: []config.EventChannel{
			{Name: "live", Uses: "sock", Inputs: map[string]string{"path": sock}, Include: []string{"tws.instruction"}},
			{Name: "dead", Uses: "sock", Inputs: map[string]string{"path": dead}, Include: []string{"tws.instruction"}},
		},
...
func TestDispatcher_CancelMidEventLeavesCursorForReplay(t *testing.T) {
	...
	d := &sessionDispatcher{
		session:  "o/r-1",
		channels: []config.EventChannel{{Name: "slow", Uses: "slow", Include: []string{"tws.instruction"}}},
...
func TestDispatcher_ReplaysFromCursorAcrossRestart(t *testing.T) {
	...
	d1, s := runtimeDispatcher("o/r-1", log, sock, "tws.instruction")
	log.Append(event.Event{SessionName: "o/r-1", Type: event.TypeInstruction, Body: "first"})
	drainOnce(d1, s)
	recvType(t, recv) // delivered once

	// A fresh dispatcher over the same log must not re-deliver the committed event.
	d2, _ := runtimeDispatcher("o/r-1", log, sock, "tws.instruction")
	drainOnce(d2, s)
	assertNoDelivery(t, recv)
