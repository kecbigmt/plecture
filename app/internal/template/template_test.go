...
func TestLoad_UserGlobalOverride(t *testing.T) {
	...
	templateDir := filepath.Join(tmpHome, ".config", "tws", "templates")
...
func TestLoad_RepoSpecificOverride(t *testing.T) {
	...
	repoDir := t.TempDir()
	templateDir := filepath.Join(repoDir, ".tws", "templates")
	if err := os.MkdirAll(templateDir, 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestRender_SessionVarsNoURL(t *testing.T) {
	...
	workdir := t.TempDir()
	twsDir := filepath.Join(workdir, ".tws", "templates")
	...
	if err := os.WriteFile(filepath.Join(twsDir, "kick.md"), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestRender_EmptyVars(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(twsDir, "bare.md"), []byte("[{{.Workflow.outputs.missing}}]"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestRender_GetGuardsOptionalVars(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(twsDir, "guard.md"), []byte(body), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestList_RepoSpecificAndDedup(t *testing.T) {
	...
	repoDir := t.TempDir()
	twsDir := filepath.Join(repoDir, ".tws", "templates")
	if err := os.MkdirAll(twsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(twsDir, "work.md"), []byte(repoContent), 0o644); err != nil {
		t.Fatal(err)
	}

	// Repo-specific template that is unique
	customContent := "---\ndescription: Deploy to staging\n---\nDeploy"
	if err := os.WriteFile(filepath.Join(twsDir, "deploy.md"), []byte(customContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestList_NoFrontmatter(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(twsDir, "plain.md"), []byte("Just a template"), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestLoad_AncestorOverlay covers the bare layout: .tws/templates lives in the
...
func TestLoad_AncestorOverlay(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(twsDir, "work.md"), []byte(ancestorContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLoad_AncestorOverlay_InnerWins(t *testing.T) {
	...
	repoDir := filepath.Join(tmpHome, "worktrees", "github.com", "org", "repo")
	if err := os.MkdirAll(filepath.Join(repoDir, ".tws", "templates"), 0o755); err != nil {
		t.Fatal(err)
	}
	outerContent := "OUTER (REPO CONTAINER) TEMPLATE"
	if err := os.WriteFile(filepath.Join(repoDir, ".tws", "templates", "work.md"), []byte(outerContent), 0o644); err != nil {
		t.Fatal(err)
	}

	worktreeDir := filepath.Join(repoDir, "session")
	if err := os.MkdirAll(filepath.Join(worktreeDir, ".tws", "templates"), 0o755); err != nil {
		t.Fatal(err)
	}
	innerContent := "INNER (WORKTREE) TEMPLATE"
	if err := os.WriteFile(filepath.Join(worktreeDir, ".tws", "templates", "work.md"), []byte(innerContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLoad_Priority(t *testing.T) {
	...
	userDir := filepath.Join(tmpHome, ".config", "tws", "templates")
