...
To track a related PR (one you opened, or a dependency you're waiting on), run `sennit subscribe <pr-url>` — its CI / review / merge events then arrive in this session (`sennit event list`).
To ask another session to act (e.g. request a re-review after pushing fixes), publish an event to it (`sennit event publish <target-session> --type user.emit ...`) — a GitHub PR comment is a public mirror, not a delivery channel, and may never reach it.
