...
func resolveSetOutputTarget(cfg *config.Config, session *domain.Session, params SetOutputParams) (target string, mutable []string, schema *jsonschema.Schema, err *Error) {
	if params.Workflow {
		...
		if !ok {
			return "", nil, nil, &Error{Code: ErrExecutionFailed, Message: fmt.Sprintf("workflow %q not found in .tws/workflows or global config", session.Workflow)}
		}
		...
		compiled, cErr := task.CompileSchema(prov.OutputsSchema, prov.ResolvedOutputsSchemaPath(), "tws:provider:"+prov.ID+":outputs")
		if cErr != nil {
	...
	if params.Task != "" {
		...
		compiled, cErr := task.CompileSchema(def.OutputsSchema, def.ResolvedOutputsSchemaPath(), "tws:task:"+taskID+":outputs")
		if cErr != nil {
