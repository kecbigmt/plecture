...
// validSessionNameRE is tws's own conservative session id charset: ids
...
// SessionGuardForOwnSession is for a per-session `tws mcp listen` socket:
// injecting the result into every spawned `tws mcp serve` child's env makes
// the socket's scope independent of whatever TWS_SESSION_GUARD the listening
