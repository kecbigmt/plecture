...
// direction is forced to Inbound whenever TWS_SESSION_NAME names a session
...
func TestEventPublishDirectionNormalization(t *testing.T) {
	...
	t.Run("cross-session publish forces inbound", func(t *testing.T) {
		t.Setenv("TWS_SESSION_NAME", "owner/other")
	...
	t.Run("same-session publish keeps requested direction", func(t *testing.T) {
		t.Setenv("TWS_SESSION_NAME", "owner/self")
...
// EventPublish stamps the session's persisted StreamID onto the event so every
// tws-core event (user notes, lifecycle, hook-published replies) carries the
// stream without the caller threading it through.
func TestEventPublishStampsStreamID(t *testing.T) {
	store := state.NewStore(t.TempDir())
	if err := store.Put(&domain.Session{Name: "owner/repo-7", StreamID: "stream-abc"}); err != nil {
		t.Fatal(err)
	}
	ev, err := EventPublish(nil, store, "owner/repo-7", EventPublishParams{Type: event.TypeUserNote, Summary: "hi"})
	if err != nil {
		t.Fatalf("publish: %v", err)
	}
	if ev.StreamID != "stream-abc" {
		t.Fatalf("event not stamped with session stream: %q", ev.StreamID)
	}

	// A session with no stream (or none in state) yields an unstamped event.
	if err := store.Put(&domain.Session{Name: "owner/repo-9"}); err != nil {
		t.Fatal(err)
	}
	ev2, err := EventPublish(nil, store, "owner/repo-9", EventPublishParams{Type: event.TypeUserNote})
	if err != nil {
		t.Fatal(err)
	}
	if ev2.StreamID != "" {
		t.Fatalf("streamless session must yield an unstamped event, got %q", ev2.StreamID)
	}
}

// A guarded orchestrator (TWS_SESSION_GUARD → cfg.SessionGuard) must not be
// able to publish into a session outside its name space, even though `tws ls`
// reveals the name.
...
// EventPageStream merges a stream across sessions, pages forward via a ULID
// keyset cursor, and never bleeds another stream in.
func TestEventPageStreamPaginatesAcrossSessions(t *testing.T) {
	store := state.NewStore(t.TempDir())
	// Two sessions in stream "s", carrying the stream via their state so the
	// publish path stamps it; plus a decoy session in another stream.
	for _, n := range []string{"o/r-1", "o/r-2"} {
		if err := store.Put(&domain.Session{Name: n, StreamID: "s"}); err != nil {
			t.Fatal(err)
		}
	}
	if err := store.Put(&domain.Session{Name: "o/r-9", StreamID: "other"}); err != nil {
		t.Fatal(err)
	}
	// Interleave appends across the two sessions so the merged order spans them.
	plan := []struct{ session, summary string }{
		{"o/r-1", "A"}, {"o/r-2", "B"}, {"o/r-1", "C"}, {"o/r-2", "D"}, {"o/r-1", "E"},
	}
	for _, p := range plan {
	...
	// Decoy in another stream must never appear.
	if _, err := EventPublish(nil, store, "o/r-9", EventPublishParams{Type: event.TypeUserNote, Summary: "X"}); err != nil {
		t.Fatal(err)
	}

	first, err := EventPageStream(store, "s", EventPageParams{Filter: event.Filter{Limit: 2}})
	if err != nil {
		t.Fatalf("first page: %v", err)
	}
	...
	second, err := EventPageStream(store, "s", EventPageParams{Cursor: first.NextCursor, Filter: event.Filter{Limit: 2}})
	if err != nil {
		t.Fatalf("second page: %v", err)
	}
	...
	third, err := EventPageStream(store, "s", EventPageParams{Cursor: second.NextCursor, Filter: event.Filter{Limit: 2}})
	if err != nil {
	...
	// The final (partial) page still carries a resume watermark pointing after E,
	// so an incremental tailer can persist it and resume. The empty page,
	// not the empty cursor, is the "caught up" signal.
	if third.NextCursor == "" {
		t.Fatal("final page must carry a resume watermark cursor")
	}

	// Resuming from the watermark with no newer events yields an empty page and
	// no further cursor — caught up, no duplicates.
	caughtUp, err := EventPageStream(store, "s", EventPageParams{Cursor: third.NextCursor, Filter: event.Filter{Limit: 2}})
	if err != nil {
...
// TestEventPageStreamWatermarkNoLimit covers the orchestrator's actual read
// shape: no --limit (Limit 0 = all). The page must still carry a resume
// watermark so .stream_cursor can advance — the truncation-only cursor it
// returned before never advanced under this command.
func TestEventPageStreamWatermarkNoLimit(t *testing.T) {
	store := state.NewStore(t.TempDir())
	if err := store.Put(&domain.Session{Name: "o/r-1", StreamID: "s"}); err != nil {
		t.Fatal(err)
	}
	for _, s := range []string{"A", "B", "C"} {
		if _, err := EventPublish(nil, store, "o/r-1", EventPublishParams{Type: event.TypeUserNote, Summary: s}); err != nil {
			t.Fatal(err)
		}
	}

	first, err := EventPageStream(store, "s", EventPageParams{})
	if err != nil {
		t.Fatalf("first page: %v", err)
	}
	if len(first.Events) != 3 || first.NextCursor == "" {
		t.Fatalf("first page = %+v cursor=%q, want 3 events + watermark", summaryList(first.Events), first.NextCursor)
	}

	// A newer event arrives; resuming from the watermark returns only it.
	if _, err := EventPublish(nil, store, "o/r-1", EventPublishParams{Type: event.TypeUserNote, Summary: "D"}); err != nil {
		t.Fatal(err)
	}
	next, err := EventPageStream(store, "s", EventPageParams{Cursor: first.NextCursor})
	if err != nil {
		t.Fatalf("resume page: %v", err)
	}
	if len(next.Events) != 1 || next.Events[0].Summary != "D" {
		t.Fatalf("resume page = %+v, want only D", summaryList(next.Events))
	}
}

func TestEventPageStreamRejectsCrossStreamCursor(t *testing.T) {
	store := state.NewStore(t.TempDir())
	if err := store.Put(&domain.Session{Name: "o/r-1", StreamID: "s"}); err != nil {
		t.Fatal(err)
	}
	for range 2 {
		if _, err := EventPublish(nil, store, "o/r-1", EventPublishParams{Type: event.TypeUserNote}); err != nil {
			t.Fatal(err)
		}
	}
	page, err := EventPageStream(store, "s", EventPageParams{Filter: event.Filter{Limit: 1}})
	if err != nil || page.NextCursor == "" {
		t.Fatalf("setup page: err=%v cursor=%q", err, page.NextCursor)
	}
	// A cursor issued for stream "s" must be rejected for stream "other".
	_, err = EventPageStream(store, "other", EventPageParams{Cursor: page.NextCursor})
	var svcErr *Error
	if !errors.As(err, &svcErr) || svcErr.Code != ErrInvalidInput {
		t.Fatalf("want ErrInvalidInput for cross-stream cursor, got %v", err)
	}
}
