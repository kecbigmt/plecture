...
// is a point-in-time snapshot, not a timeline (tws event list serves that).
...
// the same dry-run report the retired `tws check` used to give.
...
// decision-making material the retired `tws check` used to report per
// instance, so orchestrator consumers reading `tws status --json` lose
...
// StatusResult is the pure fact renderer `tws status` reports: four layers of
...
type StatusResult struct {
	...
	// the same session-wide notices `tws check` used to report (e.g. a
...
// of truth first (mirrors the old `tws show --refresh`).
...
// instead of an error — `tws status` reports facts, it doesn't fail on them.
...
// evaluateSessionActions, the same evaluation `tws check`/`tws tick` act on),
...
// StatusSummary is the default `tws status --json` shape: identity plus, per
...
// DoneWhenCell renders the `tws ls` DONE_WHEN column: "-" when no instance
