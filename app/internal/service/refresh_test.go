...
func TestRefreshSessionOutputs_RefreshesDynamicInstancesOnly(t *testing.T) {
	...
	store.Put(&domain.Session{
		Name: "owner/repo-9", WorktreePath: t.TempDir(), Workflow: "wf",
		Tasks: map[string]*contract.TaskState{
...
func TestRefreshInstanceOutputs_ProducesGroup(t *testing.T) {
	...
	store.Put(&domain.Session{
		Name: "owner/repo-8", WorktreePath: t.TempDir(), Workflow: "wf",
		Tasks: map[string]*contract.TaskState{
			"review#1": {TaskID: "review", Dynamic: true, Status: contract.TaskStatusProduced, Outputs: map[string]any{}},
		},
	})
...
// A builtin output (no [[outputs]] declared) fetches from the real worktree —
// local DoD rides the same mechanism as a remote check.
func TestRefreshInstanceOutputs_BuiltinWorktreeDirty(t *testing.T) {
	store := testStore(t)
	cfg := writeWorkflowFixture(t, t.TempDir(), "wf",
	...
	store.Put(&domain.Session{
		Name: "owner/repo-7", WorktreePath: wt, Workflow: "wf",
		Tasks: map[string]*contract.TaskState{
	...
	if len(results) != 1 || !results[0].Fetched || results[0].Name != "worktree_dirty" || results[0].Value != "1" {
		t.Fatalf("results = %+v, want worktree_dirty=1 (one untracked file)", results)
	}
}
...
func TestRefreshInstanceOutputs_NoDynamicOutputs(t *testing.T) {
	...
	cfg := writeWorkflowFixture(t, t.TempDir(), "wf",
		...
		[]nodeFixture{{id: "tmux"}})
	store.Put(&domain.Session{
		Name: "owner/repo-3", WorktreePath: t.TempDir(), Workflow: "wf",
		Tasks: map[string]*contract.TaskState{
			"tmux": {TaskID: "tmux", Status: contract.TaskStatusProduced},
		},
	})
