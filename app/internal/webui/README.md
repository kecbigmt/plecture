...
A control-plane web UI for tws session management. The `tws-web` command
...
```bash
go run ./app/cmd/tws-web                          # http://127.0.0.1:8787 (default: loopback)
go run ./app/cmd/tws-web --host 0.0.0.0 -p 8799   # expose on a private network / VPN
```

The bind address is set via `--host` / `--port` (`-p`), or `listen_addr` in
`~/.config/tws-web/config.toml`. It defaults to loopback so a fresh install
...
- **CSRF** (always on): mutating POSTs require **same-origin** (Origin/Referer
  host == Host) and a **double-submit token** (`tws_csrf` cookie ==
...
`tws-web` CI workflow runs `pnpm build` and fails if it produces an
