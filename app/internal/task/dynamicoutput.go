...
func FetchOutput(cfg *config.Config, src config.DynamicOutput, ctx RenderContext) (map[string]string, error) {
	...
	stdout, stderr, runErr := execHostScript(cmdStr, ctx.Session.WorktreePath)
	if runErr != nil {
