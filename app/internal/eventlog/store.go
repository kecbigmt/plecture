...
// the tws event bus. It is the source of truth: a session's log.jsonl is the
...
// NewStore creates a Store. If dir is empty it defaults to ~/.local/share/tws
...
// `tws destroy` deleting the session's state.json entry. data is an opaque
