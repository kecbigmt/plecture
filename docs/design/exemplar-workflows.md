...
referenced task, channel, workspace provider, and environment. The metadata also
names placeholders that must be replaced at scaffold time because they are
team-local policy rather than catalog-owned config.
...
kind = "task"
id = "envfile"
description = "Provide a local task that returns environment values consumed by the selected runtime."

[[placeholders]]
kind = "task"
id = "slack_thread"
description = "Provide a local task that returns channel_id and thread_ts for the team conversation."

[[placeholders]]
kind = "task"
id = "initial_task"
description = "Replace with the team's selected initial-instruction task, such as a Codex or Claude starter task."
```
...
| Field | Required | Meaning |
|---|---:|---|
| `kind` | yes | One of `workspace_provider`, `environment`, `task`, or `channel`. |
...
- every workflow-level workspace provider and environment reference, every node
  `uses`, and every event channel `uses` is declared in either `[[references]]`
  or `[[placeholders]]`;
...
- a node whose `uses` is a task placeholder declares an explicit `id` matching
  the placeholder id;
- no `[[references]]` or `[[placeholders]]` entry is unused by the workflow
  template;
...
ids carried by workflow inputs rather than by workflow grammar positions. In
the OKF goal review exemplar, the `task` input enum contains `goal_review`, and
`goal_review` must resolve as a task supplied by the OKF plugin.

If a plugin is not enabled, the error names the plugin to enable:

```text
exemplar official/okf-goal-review requires task "codex_exec" from plugin official/codex
```
...
```text
exemplar official/okf-goal-review needs task placeholder "slack_thread": provide a local task that returns channel_id and thread_ts for the team conversation
```
...
plect workflow init goal-review --from official/okf-goal-review \
  --replace task:slack_thread=team_slack_thread \
  --replace task:initial_task=codex_initial_prompt
```
...
it changes only ids in the workflow grammar position matching the declared
placeholder kind. For task placeholders used by workflow nodes, replacement
changes the node's `uses` value and preserves the node `id`.
...
The `--replace task:slack_thread=team_slack_thread` result keeps the node id as
...
continues to own `local-okf`, `okf_goal`, `pursue_goal`, `goal_review`, and
`goal_bootstrap`. The workflow that combines OKF with a terminal multiplexer,
an agent runtime, and team conversation delivery lives at:

```text
exemplars/workflows/okf-goal-review/
```

The exemplar template uses `local-okf` as its workspace provider, starts the
selected terminal and agent runtime tasks, and wires event channels for runtime
delivery and team conversation delivery. `slack_thread` and `initial_task` are
scaffold-time placeholders unless the catalog later grows plugin-owned
provider-neutral tasks for those roles. `envfile` is also a scaffold-time
placeholder when the selected runtime expects a local environment task.

The scaffolded user-owned copy may use any equivalent local task ids. The
...
environment, task, and channel id is provided by the same plugin. Cross-plugin
composition belongs in an exemplar package or in user-owned workflow config.
