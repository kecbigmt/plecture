...
| `tasks/work.md` | Different instruction, team-specific completion gates, and local chain attachment. | 3 | A task document has no nesting joint, so a team-specific `work` was a document of its own that replaced the shipped one by id. The `github` plugin no longer ships a `work` document to replace — a host now authors `tasks/work.md` from scratch (see `docs/language/tasks.md`). |
