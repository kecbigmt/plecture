...
The OKF plugin currently carries a `goal_review` workflow composition that
references task and channel ids outside the OKF plugin. Some of those ids belong
to other provider plugins, and some are local user configuration. That makes the
plugin unresolvable on a fresh install and blurs the boundary between a
capability package and a team's workflow policy.
...
task, channel, workspace provider, or environment. `plect workflow init <name> --from
...
The OKF goal review composition moves from plugin-mounted workflow content to an
exemplar workflow package. Local-only references such as the team Slack-thread
task and initial-instruction task are scaffold-time placeholders, not hidden
...
Existing plugin-mounted cross-plugin workflows need a migration to exemplar
packages when the scaffold implementation lands. Because Plecture is pre-1.0,
the implementation carries a one-time migration procedure rather than a
compatibility shim.
