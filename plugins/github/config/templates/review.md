...
{{- if $work}}
8. This review gates the work session `{{$work}}`. Record one action per
   pending judge id ({{$judges}}) with a reason. Find the exact task instance
   and judge state with `plect status {{$work}} --json`, then:
   `plect judge approve {{$work}} <task-instance> <judge-id> --reason "<reason>"`
   Use `plect judge request-changes {{$work}} <task-instance> <judge-id> --reason "<reason>"`
