...
// flows through it into tws's per-session event log, so a session's timeline
...
// and the adapter already knows the tws session_name via its broker — so all
// capture lives here and channel-server stays source-independent (it must not
// know about tws sessions). Recording is done by exec'ing the `tws` CLI (no
// import of tws/app) and is best-effort: a failure is logged, never blocking
// delivery.

// captureArgs builds the `tws event publish` argument vector. Captured event
...
func (a *Adapter) publishEvent(sessionName, eventType, source, direction, summary, body string, meta map[string]string) {
	...
	cmd := exec.Command("tws", captureArgs(sessionName, eventType, source, direction, summary, body, meta)...)
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		a.logger.Warn("tws event capture failed",
