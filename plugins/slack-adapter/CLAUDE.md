...
- Persists subscriptions to `$XDG_STATE_HOME/slack-adapter/subscribers.json` via atomic write and reloads them at startup (makes broker restarts transparent to tws)
...
- Must not import `tws/app` code
- Imports the `tws/contracts/channel-protocol` module to share protocol types
- Does not depend on tws's state schema (`tws/contracts/state`) — removed in Phase 2
- May import `tws/plugins/channel-server/server` only in test code (`server.NewSocketListener` etc.)
...
- The protocol is defined in the `tws/contracts/channel-protocol` module, a shared contract owned by neither service
...
|---|---|---|
| `GET /info` | Returns workspace name and channel ID | tws task (`slack_thread`) |
| `POST /threads` | Creates a Slack thread | tws task (`slack_thread`) |
| `POST /messages` | Posts a message to a thread | `slack_thread` cleanup, `claude-slack-notify.sh` |
| `POST /subscribe` / `DELETE /subscribe?thread_ts=...` | Register/unregister a subscription | tws task (`slack_subscribe`) |
| `GET /subscribers` | Lists subscriptions (for healthcheck) | tws task (`slack_subscribe`) |
...
socket_client_test and socket_router_test run integration tests using `tws/plugins/channel-server/server.NewSocketListener`.
