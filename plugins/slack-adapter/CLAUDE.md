...
- Persists subscriptions to `$XDG_STATE_HOME/slack-adapter/subscribers.json` via atomic write and reloads them at startup (makes broker restarts transparent to plecture)
...
- Must not import `plecture/app` code
- Imports the `plecture/contracts/channel-protocol` module to share protocol types
- Does not depend on plecture's state schema (`plecture/contracts/state`) — removed in Phase 2
- May import `plecture/plugins/channel-server/server` only in test code (`server.NewSocketListener` etc.)
...
- The protocol is defined in the `plecture/contracts/channel-protocol` module, a shared contract owned by neither service
...
|---|---|---|
| `GET /info` | Returns workspace name and channel ID | plecture task (`slack_thread`) |
| `POST /threads` | Creates a Slack thread | plecture task (`slack_thread`) |
| `POST /messages` | Posts a message to a thread | `slack_thread` cleanup, `claude-slack-notify.sh` |
| `POST /subscribe` / `DELETE /subscribe?thread_ts=...` | Register/unregister a subscription | plecture task (`slack_subscribe`) |
| `GET /subscribers` | Lists subscriptions (for healthcheck) | plecture task (`slack_subscribe`) |
...
socket_client_test and socket_router_test run integration tests using `plecture/plugins/channel-server/server.NewSocketListener`.
