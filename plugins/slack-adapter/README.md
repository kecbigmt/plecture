...
slack-adapter never reads tws's state.json. Every subscription is registered
...
Unsubscribes. A missing `thread_ts` is a no-op (`204`). Called from `tws
...
a broker restart dropped it, the healthcheck goes non-zero and the next `tws
up` re-runs setup.
...
without any action on the tws side. A failed read-back (missing or corrupt
