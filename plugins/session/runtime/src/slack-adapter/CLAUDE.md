...
- May import `plect/plugins/session/runtime/src/channel-server/server` only in test code (`server.NewSocketListener` etc.)
...
socket_client_test and socket_router_test run integration tests using `plect/plugins/session/runtime/src/channel-server/server.NewSocketListener`.
