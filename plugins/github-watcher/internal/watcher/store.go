...
// watcher imports no sennit core internals.
package watcher
...
// discarded on load: the github_watch task re-subscribes on the next `sennit up`,
// so there is nothing to migrate.
const registryVersion = 2

// Subscription is one watched resource bound to a sennit session. A session may
...
func (s *Store) Subscribe(sub Subscription) error {
	return s.update(func(r *registry) error {
		...
		if existing, ok := r.Subscriptions[key]; ok {
			...
			// `sennit subscribe` (which omits --branch) from wiping the branch a
...
func (s *Store) load() (*registry, error) {
	...
	// task re-subscribes on the next `sennit up`. Return a fresh empty registry
