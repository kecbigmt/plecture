...
func TestConfigureDeliveryRequiresBusUnlessLegacyOptIn(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected missing SENNIT_BUS_SOCKET to fail without legacy opt-in")
	}
	if delivery != "" {
...
func TestConfigureDeliveryUsesBusWhenSocketIsSet(t *testing.T) {
	var poller watcher.Poller
	delivery, err := configureDelivery(&poller, "/tmp/sennit-bus.sock", "secret", "http://127.0.0.1:7890/notify", true)
	if err != nil {
		t.Fatal(err)
	}
	if delivery != "bus" {
