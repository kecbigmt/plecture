// github-watcher is the resident GitHub watcher plugin for tws.
//
...
func cmdServe(args []string) error {
	...
	allowLegacy := fs.Bool("allow-legacy-notify", false, "opt into the deprecated POST /notify delivery when TWS_BUS_SOCKET is unset (dual-run rollback)")
	dataDir := fs.String("data-dir", "", "override data directory")
	if err := fs.Parse(args); err != nil {
		return err
	}
	...
	delivery, err := configureDelivery(poller, busSocket, os.Getenv("TWS_BUS_TOKEN"), *notifyURL, *allowLegacy)
	if err != nil {
		return err
	}
	switch delivery {
	...
	case "notify":
		logger.Warn("github-watcher serving in DEPRECATED legacy push mode (set TWS_BUS_SOCKET to use the event bus)",
...
func configureDelivery(poller *watcher.Poller, busSocket, busToken, notifyURL string, allowLegacy bool) (string, error) {
	...
	return "", fmt.Errorf("TWS_BUS_SOCKET is unset: set it to use the event bus, or pass --allow-legacy-notify to opt into the deprecated POST /notify path")
}
