...
- The `resource observe` CLI subcommand is `thread_state`'s `observe` action (the config language requires one); it never contacts the resident service, since the observer's `state_schema` is empty and there is nothing live to fetch
